...
/*
u mainu stvaramo binarno stablo pretraživanja koristeći brojeve iz prvog niza,
ispisujemo elemente stabla po inorder redu od najmanjeg do najvećeg i spremamo ih u datoteku,
...
korisnik unosi raspon brojeva između 10 i 90,
generiramo 10 nasumičnih brojeva unutar toga raspona i spremamo ih u drugi niz ovo array2,
stvaramo novo stablo koristeći brojeve iz array2,
...
//ova funkcija stvara novi čvor s određenom vrijednošću i vraća pokazivač na taj čvor
Position CreateElement(int element)
{
	Position new = NULL; //deklariramo pokazivač new tipa Position i postavlja ga na NULL
	new = (Position)malloc(sizeof(tree)); //dinamički dodjeljujemo memoriju za novi čvor veličine strukture tree i pokazivač new pokazuje na tu memoriju
	new->El = element; //postavljamo vrijednost čvora na element, koji je proslijeđen kao argument funkcije
	new->L = NULL; //postavlja lijevo dijete čvora na NULL
	new->R = NULL; //postavlja desno dijete čvora na NULL
	return new; //vraća pokazivač na novostvoreni čvor
}
...
//obilazimo stablo i ispisiujemo elemenate od najmanjeg do najvećeg
int Inorder(Position current)
{
	if (current)
	{
...
//funkcija generira i vraća nasumični broj unutar određenog raspona
int RandomNum(int min, int max)
{
	int n = 0;
	n = max - min + 1;
